# by @itsz_krish_babess (@itsz_krish_babess)
import io
import os
...
async def imirror(event):  # sourcery no-metrics
    ...
    arankevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "arankuserbot.webp"
        f_format = "webp"
    else:
        filename = "arankuserbot.jpg"
        f_format = "jpeg"
    try:
        imag = await Convert.to_image(
            arankevent, reply, dirct="./temp", file="imirror.png", noedits=True
        )
        if imag[1] is None:
        ...
        return await edit_delete(arankevent, f"**Error in identifying image:**\n__{e}__")
...
async def irotate(event):
    ...
    if mediatype == "Sticker" and reply.document.mime_type == "appliarankion/i-tgsticker":
        return await edit_delete(
            event,
            "__Reply to photo or sticker to rotate it with given angle. Animated sticker is not supported__",
        )
    args = event.pattern_match.group(1)
    arankevent = await edit_or_reply(event, "__Rotating the replied media...__")
    imag = await Convert.to_image(
        arankevent, reply, dirct="./temp", file="irotate.png", noedits=True
    )
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def iresize(event):
    ...
    arankevent = await edit_or_reply(event, "__Resizeing the replied media...__")
    imag = await Convert.to_image(
        arankevent, reply, dirct="./temp", file="iresize.png", noedits=True
    )
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
    ...
    if len(args) == 1:
        try:
            ...
            return await edit_delete(arankevent, "**Error:**\n__Invalid dimension.__")
    else:
        try:
            nw = int(args[0])
        except ValueError:
    ...
    try:
        ...
        return await edit_delete(arankevent, f"**Error:** __While resizing.\n{e}__")
    await event.delete()
    img = io.BytesIO()
    img.name = "arankUserbot.png"
    image.save(img, "PNG")
...
async def square_cmd(event):
    ...
    arankevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await Convert.to_image(
            arankevent, reply, dirct="./temp", file="square.png", noedits=True
        )
        if imag[1] is None:
            return await edit_delete(
                imag[0], "__Unable to extract image from the replied message.__"
...
async def pic_gifcmd(event):
    ...
    if args := event.pattern_match.group(1):
        ...
        pix = 100
    arankevent = await edit_or_reply(event, "__🎞Dotifying image...__")
    imag = await Convert.to_image(
        arankevent, reply, dirct="./temp", file="dotify.png", noedits=True
    )
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
