"""
created by @itsz_krish_babess
Idea by @itsz_radha_3030
"""
...
async def echo(event):
    ...
    arankevent = await edit_or_reply(event, "`Adding Echo to user...`")
    user, rank = await get_user_from_event(event, arankevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(arankevent, f"**Error:**\n`{e}`")
    else:
        await edit_or_reply(arankevent, "Hi")
...
@arankub.arank_cmd(incoming=True, edited=False)
