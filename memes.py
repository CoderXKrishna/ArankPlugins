...
# arankUserbot module for having some fun with people.
import asyncio
import random
import re
...
from userbot.helpers import arankmemes, get_user_from_event
from userbot.helpers.utils import _arankutils, parse_pre
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay arankuserbot",
            "{tr}milksay arankuserbot",
            "{tr}ghostbustersghostbusterssay arankuserbot",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await arankmemes.slap(replied_user, event, mention)
    try:
...
async def decide(event):
    ...
    await _arankutils.unsavegif(event, sandy)
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", f" {random.choice(arankmemes.UWUS)}", reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += f" {random.choice(arankmemes.UWUS)}"
...
async def gbun(event):
    ...
    arankevent = await edit_or_reply(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await arankevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await arankevent.edit(mention)
