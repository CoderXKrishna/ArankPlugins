# credits to @itsz_krish_babess and @itsz_krish_babess
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await arankub(unblock("hazmat_suit_bot"))
            flag = await conv.send_file(output[1])
...
async def _(event):
    ...
    arank = f"https://telegra.ph{response[0]}"
    arank = await awooify(arank)
    await output[0].delete()
    await event.client.send_file(event.chat_id, arank, reply_to=replied)
